...
The data access layer (DAL) is the component in HopsFS that replaces all interactions with in-memory metadata objects with transactions to the metadata storage cluster.
It is divided into two different packages, the data access layer itself, which is a collection of interfaces and common behaviour and the implementation layer, \texttt{hops-metadata-dal-impl-ndb}, which implements the interfaces and connection parameters for the MySQL Cluster database software.
While this theoretically allows NewSQL databases other than MySQL Cluster to be used as the metadata storage system, MySQL Cluster is the only backend currently implemented.
As such, any references to the DAL in this document refer to both the common and implementation specific parts.

The DAL provides two services to Hops: 
\begin{inparaenum}[1)]
    \item it manages the life-cycle of database connections for both MySQL and NDB, and
    \item it provides a high-level object based API that clients (HopsFS and HopsYARN) use to manage transactions and locking.
\end{inparaenum}

\paragraph{Connection Management}

The DAL maintains a set (pool) of connections to the metadata storage cluster.
