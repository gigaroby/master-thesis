...
To solve the mismatch of HDFS with cloud environments, the Hops project provides a scalable, cloud-ready, protocol-compatible distribution of HDFS called HopsFS.
...
As demonstrated in \cite{niazi2016hopsfs} on a workload trace provided by Spotify, the improvements brought by the increased scalability allow HopsFS to perform 16 times the metadata operations in the same amount of time.
...
\begin{itemize}
    \item investigate the properties of asynchronous replication in the metadata storage layer
    \item define the changes in behavior to the filesystem as a result of this work, if any
    \item implement the changes in HopsFS
\end{itemize}
...
\item MySQL cluster and NDB, to understand the technology behind the metadata storage system with a focus on the geographic-replication features
\end{inparaenum}
